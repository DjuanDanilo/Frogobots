...
colorModeButtonDOM.addEventListener("click", () => {
  if (settings.mode === "dark") {
    settings.mode = "light";
    colorModeButtonDOM.innerText = "Tema Escuro";
  } else {
    settings.mode = "dark";
    colorModeButtonDOM.innerText = "Tema Claro";
  }
  draw();
});
...
function drawBackgroundSky() {
  ...
  if (settings.mode === "dark") {
    ...
    state.stars.forEach((star) => {
      ctx.fillRect(star.x, star.y, 2.2, 1);
    });
  }
}
...
function drawGorillaFace(player) {
  // Face
  ctx.fillStyle = settings.mode === "light" ? "green" : "lightgreen";
  ctx.beginPath();
...
function drawGorillaThoughtBubbles(player) {
  if (state.phase === "aiming") {
    ...
    if (currentPlayerIsComputer) {
      ...
      ctx.fillText("Pode acertar se quiser 😝", 0, -90);
